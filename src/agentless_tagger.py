...
def _get_update_account_count():
    try:
        update_account_count = int(sys.argv[2])

    except IndexError:
        sys.exit(
            "Error: Please enter the number of accounts you want to modify. Use 0 update all accounts"
        )

    return update_account_count
...
def _build_new_cwp_config_payload(
    update_account_count, new_cwp_configs, new_custom_tags
):
...
def main():
    update_account_count = _get_update_account_count()
